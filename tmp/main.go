...
func main() {
	...
	netWireFile, err := os.Open("../testdata/expected.binpb") // simple.binpb
	must(err)
	...
	// writing the files
	must(os.WriteFile(path.Join("res", "test.h"), hFileBuf.Bytes(), 0666)) // simple_out.h
	must(os.WriteFile(path.Join("res", "test.c"), wFileBuf.Bytes(), 0666)) // simple_out.c
}
